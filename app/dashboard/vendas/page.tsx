    ...
    >
      ...
      {progress !== undefined && (
        <div className="mt-3 h-1.5 bg-slate-700 rounded-full overflow-hidden">
          <div className={`h-full ${progressColor} transition-all duration-1000`} style={{ width: `${Math.min(progress, 100)}%` }} />
        </div>
      )}
    </div>
  )
...
export default function DashboardVendasPage() {
  ...
  // ✅ HOOKS ESPECIALIZADOS COM DEBOUNCE
  const { vendas } = useVendas(anoSelecionado, [mesSelecionado])
  ...
  const mixRatio = useMemo(() => {
    ...
    return { toxina: toxinaAjustada, preenchedor, biotech, formatted: `${format(ratioT)} : ${format(ratioP)} : ${format(ratioB)}` }
  }, [vendasPorCategoria])
  ...
  const categoriaDefasada = useMemo(() => {
    ...
    const PATTERNS = [[1, 3, 1], [1, 4, 1], [1, 4, 2], [2, 4, 1], [2, 5, 2]]
    const TIE_EPS = 0.03
    const real = [toxinaAjustada, preenchedor, biotech]

    const bestScaleK = (realArr: number[], pattern: number[]) => {
      let num = 0, den = 0
      for (let i = 0; i < 3; i++) { num += realArr[i] * pattern[i]; den += pattern[i] * pattern[i] }
      return den === 0 ? 0 : num / den
    }

    const mseAfterScale = (realArr: number[], pattern: number[], k: number) => {
      let sum = 0
      for (let i = 0; i < 3; i++) { const diff = realArr[i] - k * pattern[i]; sum += diff * diff }
      return sum / 3
    }

    const deficiencyForPattern = (realArr: number[], pattern: number[], k: number) => {
      const expected = pattern.map(v => v * k)
      const fulfillment = expected.map((exp, i) => exp <= 0 ? Number.POSITIVE_INFINITY : realArr[i] / exp)
    ...
    if (relDiff <= TIE_EPS) {
      return { categoria: `${CATEGORIES[first].nome}, ${CATEGORIES[second].nome}`, color: 'amber', indices: [first, second] }
    }
            ...
            <ResponsiveContainer width="100%" height="100%">
              {viewMode === 'daily' ? (
                ...
                <ComposedChart data={chartData}>
                  ...
                  <Line type="monotone" dataKey="acumuladoMeta" stroke="#64748b" strokeWidth={2} strokeDasharray="8 4" dot={false} name="acumuladoMeta" />
                  <Area type="monotone" dataKey="acumuladoReal" stroke="#00ff9d" fill="url(#colorReal)" strokeWidth={3} name="acumuladoReal" dot={{ fill: '#00ff9d', strokeWidth: 0, r: 2 }} activeDot={{ fill: '#00ff9d', strokeWidth: 2, stroke: '#fff', r: 6 }} />
                </ComposedChart>
              )}
            </ResponsiveContainer>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-[2]">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-slate-400 text-sm font-bold uppercase tracking-wider">Mix de Produtos - {tituloMix}</h3>
                <span className="text-xs text-slate-500">Dados Acumulados</span>
              </div>

              {/* Filtro de Meses do Mix */}
              <div className="mb-6 pb-6 border-b border-slate-700/50">
                <label className="text-xs text-slate-400 uppercase mb-3 block">Meses (clique para selecionar múltiplos)</label>
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="bg-black/20 border-l-4 border-cyan-400 rounded-lg p-4">
                    <span className="text-3xl font-bold text-cyan-400 font-mono">{mixRatio.toxina}</span>
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <div className="bg-black/20 border-l-4 border-green-400 rounded-lg p-4">
                    <span className="text-3xl font-bold text-green-400 font-mono">{mixRatio.preenchedor}</span>
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <div className="bg-black/20 border-l-4 border-purple-400 rounded-lg p-4">
                    <span className="text-3xl font-bold text-purple-400 font-mono">{mixRatio.biotech}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-cyan-500/5 border border-dashed border-cyan-500/30 rounded-xl p-6 text-center">
                ...
                <div className="text-xs text-cyan-400 uppercase tracking-widest mt-2">Proporção Atual</div>
              </div>
                ...
                <div className={`mt-4 rounded-xl p-4 border ${
                  ...
                  <div className="flex items-center gap-3">
                    <AlertTriangle className={`w-5 h-5 flex-shrink-0 ${
                      ...
                      categoriaDefasada.color === 'purple' ? 'text-purple-400' : 'text-amber-400'
                    }`} />
                    <div>
                      <div className="text-xs text-slate-400 uppercase tracking-wider">Categoria Defasada</div>
                      <div className={`font-semibold ${
                        categoriaDefasada.color === 'cyan' ? 'text-cyan-400' :
                        categoriaDefasada.color === 'green' ? 'text-green-400' :
                        categoriaDefasada.color === 'purple' ? 'text-purple-400' : 'text-amber-400'
                      }`}>{categoriaDefasada.categoria}</div>
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Lado Direito - Legendas */}
            <div className="flex-1 border-l border-slate-700/50 pl-6">
              <div className="text-xs text-slate-400 uppercase tracking-wider font-bold mb-4">Referências de Mix</div>
            ...
            <div className="bg-slate-900 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-auto border border-slate-700" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="bg-black rounded-lg p-4 font-mono text-sm text-green-400">Meta do Dia = (Meta Total - Realizado) / Dias Úteis Restantes</div>
                </div>
                <div>
                  ...
                  <div className="bg-black rounded-lg p-4 font-mono text-sm text-green-400">Projeção R$ = (Realizado / Dias Úteis Corridos) × Total Dias Úteis</div>
                </div>
                <div>

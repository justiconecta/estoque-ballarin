...
import type { Despesa, TipoDespesa } from '@/types/database'
...
// ============ FORMATTERS (fora do componente - nunca recria) ============
...
// ✅ FinanceCard memoizado
const FinanceCard = React.memo(function FinanceCard({ 
  title, 
  value, 
  variant = 'default' 
}: { 
  title: string
  value: string | number
  variant?: 'default' | 'cyan' | 'green' | 'red' | 'purple' 
}) {
...
const MesButton = React.memo(function MesButton({
  ...
  const handleClick = useCallback(() => onToggle(mes), [mes, onToggle])
...
// ✅ FiltroPeriodo memoizado
const FiltroPeriodo = React.memo(function FiltroPeriodo({ 
  anos, 
  setAnos, 
  mesesSelecionados, 
  onMesToggle 
          ...
          <div className="flex flex-wrap gap-2">
            {MESES.map((nome, index) => (
              <MesButton
                ...
                mes={index + 1}
                nome={nome}
                selecionado={mesesSelecionados.includes(index + 1)}
                onToggle={onMesToggle}
              />
            ))}
          </div>
        </div>
...
const VendaRow = React.memo(function VendaRow({ venda }: { venda: any }) {
  ...
  return (
    <tr className="border-b border-gray-100 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30">
      <td className="px-4 py-3 text-gray-900 dark:text-white">{formatDateBR(venda.data_venda)}</td>
      <td className="px-4 py-3 text-gray-600 dark:text-slate-400">{venda.pacientes?.nome_completo || 'N/A'}</td>
      <td className="px-4 py-3 text-gray-600 dark:text-slate-400">{venda.profissional?.nome || '—'}</td>
      ...
      <td className="px-4 py-3 text-right text-gray-900 dark:text-white">{formatCurrency(venda.preco_total)}</td>
      <td className="px-4 py-3 text-right text-orange-600 dark:text-orange-400">{venda.desconto_valor > 0 ? formatCurrency(venda.desconto_valor) : '-'}</td>
      <td className="px-4 py-3 text-right font-bold text-cyan-600 dark:text-cyan-400">{formatCurrency(venda.preco_final)}</td>
      <td className="px-4 py-3 text-right text-green-600 dark:text-green-400">{formatPercent(venda.margem_percentual_final || venda.margem_percentual || 0)}</td>
      <td className="px-4 py-3 text-center">
        <span className={`px-2 py-1 rounded text-xs font-medium ${venda.metodo_pagamento === 'PIX' ? 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400' :
            venda.metodo_pagamento === 'Débito' ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400' :
...
const DespesaRow = React.memo(function DespesaRow({ despesa }: { despesa: Despesa }) {
  const tipoBadgeClass = useMemo(() => {
    switch(despesa.tipo) {
      ...
      case 'Custo Variável': return 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400'
      default: return 'bg-gray-100 dark:bg-slate-600 text-gray-600 dark:text-slate-400'
    }
  }, [despesa.tipo])
  ...
  return (
    <tr className="border-b border-gray-100 dark:border-slate-700/50">
      ...
      <td className="px-4 py-3 text-gray-900 dark:text-white">{despesa.categoria}</td>
      <td className="px-4 py-3 text-gray-600 dark:text-slate-400">{despesa.item}</td>
      <td className="px-4 py-3 text-right text-cyan-600 dark:text-cyan-400">{formatCurrency(despesa.valor_mensal)}</td>
    </tr>
  )
})
...
const ProfissionalRow = React.memo(function ProfissionalRow({
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg gap-4">
      <div className="flex-1">
        <span className="text-gray-900 dark:text-white font-medium">{profissional.nome}</span>
        <span className="text-sm text-gray-500 dark:text-slate-400 ml-2">({profissional.horas_semanais}h/semana)</span>
      </div>
      {isEditing ? (
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-cyan-600 dark:text-cyan-400 font-medium">
            {profissional.percentual_profissional != null ? `${profissional.percentual_profissional}%` : '—'}
          </span>
...
const MetaRow = React.memo(function MetaRow({ meta }: { meta: any }) {
  ...
  const projecaoClass = useMemo(() => {
    ...
    if (meta.projecao >= 70) return 'text-yellow-600 dark:text-yellow-400'
    return 'text-red-500 dark:text-red-400'
  }, [meta.projecao])

  return (
    <tr className="border-b border-gray-100 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30">
      <td className="px-4 py-3 text-gray-900 dark:text-white font-medium">{meta.produto}</td>
...
// ✅ AbaGestaoSKUs memoizada
const AbaGestaoSKUs = React.memo(function AbaGestaoSKUs({ 
  skus, 
  editandoId, 
  editForm, 
  setEditForm, 
...
// ✅ AbaDespesas memoizada
const AbaDespesas = React.memo(function AbaDespesas({ 
  despesas, 
  total, 
  mostrarForm, 
  setMostrarForm, 
      ...
      {mostrarForm && (
        <div className="bg-cyan-50 dark:bg-cyan-500/10 border border-cyan-200 dark:border-cyan-500/30 rounded-lg p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
            ...
            <InputLocal label="Item" value={novaDespesa.item} onChange={handleItemChange} />
            <InputLocal label="Valor" type="number" value={novaDespesa.valor} onChange={handleValorChange} />
          </div>
...
// ✅ AbaParametros memoizada
const AbaParametros = React.memo(function AbaParametros({ 
  parametros, 
  calculados, 
  onUpdate, 
  profissionais, 
  ...
  const handleEditProfissional = useCallback((p: any) => {
    ...
    setEditFormProfissional({ 
      ...
      perfil: p.perfil ?? '', 
      duracao_servico: p.duracao_servico ?? '' 
    })
  }, [])

  const handleSaveProfissional = useCallback(async (id: number) => {
  ...
  // Handlers memoizados para parâmetros
  const handleNumeroSalas = useCallback((v: number) => onUpdate({ numero_salas: v }), [onUpdate])
  const handleHorasDia = useCallback((v: number) => onUpdate({ horas_trabalho_dia: v }), [onUpdate])
  const handleDuracaoMedia = useCallback((v: number) => onUpdate({ duracao_media_servico_horas: v }), [onUpdate])
  const handleAliquota = useCallback((v: number) => onUpdate({ aliquota_impostos_pct: v }), [onUpdate])
  const handleTaxaCartao = useCallback((v: number) => onUpdate({ taxa_cartao_pct: v }), [onUpdate])
  const handleModernInova = useCallback((v: number) => onUpdate({ modern_inova: v }), [onUpdate])
  const handleFatorCorrecao = useCallback((v: number) => onUpdate({ fator_correcao_marca: v }), [onUpdate])
  const handleCustoHora = useCallback((v: number) => onUpdate({ custo_hora: v }), [onUpdate])
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-slate-700 p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <InputLocal label="Número de Salas" type="number" value={parametros.numero_salas} onChange={handleNumeroSalas} />
          <InputLocal label="Horas/Dia" type="number" value={parametros.horas_trabalho_dia} onChange={handleHorasDia} />
          <InputLocal label="Duração Média Serviço (h)" type="number" step="0.1" value={parametros.duracao_media_servico_horas} onChange={handleDuracaoMedia} />
          <InputLocal label="Alíquota Impostos (%)" type="number" value={parametros.aliquota_impostos_pct} onChange={handleAliquota} />
          <InputLocal label="Taxa Cartão (%)" type="number" value={parametros.taxa_cartao_pct} onChange={handleTaxaCartao} />
          <InputLocal label="Modernização e Inovação (%)" type="number" value={parametros.modern_inova} onChange={handleModernInova} />
          <InputLocal label="Fator Correção Marca (%)" type="number" value={parametros.fator_correcao_marca} onChange={handleFatorCorrecao} />
          <InputLocal label="Custo/Hora (R$)" type="number" value={parametros.custo_hora} onChange={handleCustoHora} />
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-slate-700 p-6 shadow-sm">
        ...
        <div className="space-y-3">
          ...
          {profissionais.length === 0 && (
            <p className="text-gray-500 dark:text-slate-400 text-sm text-center py-4">Nenhum profissional cadastrado</p>
          )}
        </div>
      </div>
...
// ✅ AbaMetas memoizada
const AbaMetas = React.memo(function AbaMetas({ 
  metas, 
  tituloPeriodo, 
  parametros, 
  dreCalc, 
...
// ✅ AbaControle memoizada
const AbaControle = React.memo(function AbaControle({ 
  controle, 
  dre, 
  parametros, 
  diasUteis, 
        ...
        <div className="mt-4 p-4 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-lg border border-amber-200 dark:border-amber-700/50">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-amber-600 dark:text-amber-500 mt-1">Para atingir a meta com margem atual de {formatPercent(calculos.margemAtualPct * 100)}</p>
            </div>
            <span className="text-2xl font-bold text-amber-700 dark:text-amber-300">{formatCurrency(calculos.faturamentoNecessario)}</span>
          </div>
          <div className="mt-3 pt-3 border-t border-amber-200 dark:border-amber-700/50 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
            <div><span className="text-amber-600 dark:text-amber-500">Alvo Margem:</span><span className="ml-1 font-medium text-amber-800 dark:text-amber-300">{formatCurrency(calculos.alvoMargem)}</span></div>
            <div><span className="text-amber-600 dark:text-amber-500">Reservas ({calculos.modernInova}%):</span><span className="ml-1 font-medium text-amber-800 dark:text-amber-300">{formatCurrency(calculos.reservasInovacao)}</span></div>
            <div><span className="text-amber-600 dark:text-amber-500">% Inovação:</span><span className="ml-1 font-medium text-amber-800 dark:text-amber-300">{calculos.modernInova}%</span></div>
            <div><span className="text-amber-600 dark:text-amber-500">Desp. Fixas:</span><span className="ml-1 font-medium text-amber-800 dark:text-amber-300">{formatCurrency(calculos.despesasFixas)}</span></div>
          </div>
        </div>
      </div>
...
// ✅ AbaDRE memoizada
const AbaDRE = React.memo(function AbaDRE({ dre, tituloPeriodo }: { dre: any; tituloPeriodo: string }) {
  return (
...
export default function FinanceiroPage() {
  ...
  // ============ HANDLERS MEMOIZADOS ============
  const showFeedbackSKU = useCallback((tipo: 'success' | 'error', mensagem: string) => {
  ...
  const handleCancelEditSKU = useCallback(() => {
    setEditandoSKU(null)
    setEditFormSKU({})
  }, [])

  const handleSaveSKU = useCallback(async (id_sku: number) => {
    try {
  ...
  }, [editFormSKU, showFeedbackSKU, refreshData])

  const handleAdicionarDespesa = useCallback(async () => {
  ...
  }, [novaDespesa, refreshData])

  const handleRemoverProfissional = useCallback(async (id: number) => {
    try {
  ...
  const handleUpdateProfissional = useCallback(async (id: number, updates: any) => {
    try {
  ...
  const handleUpdateParametros = useCallback(async (updates: Record<string, number | string | null>) => {
    try {
  ...
  const handleUpdateMetaMensal = useCallback(async (novaMeta: number) => {
    try {
  ...
  const handleVendaSuccess = useCallback(() => {
    setShowNovaVendaModal(false)
    clearVendasCache()
    reloadVendas()
    refreshData(['produtos'])
  }, [clearVendasCache, reloadVendas, refreshData])

  const handleOpenNovaVenda = useCallback(() => setShowNovaVendaModal(true), [])
  const handleCloseNovaVenda = useCallback(() => setShowNovaVendaModal(false), [])
